 ...
 * 
 * @method shouldIgnoreScheamNamespace
 * @param {Object} schema   The parsed WSDL Schema object.
 */
WSDL.prototype.shouldIgnoreScheamNamespace = function(schema) {
...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  if (body.Fault) {
    var error = new Error(body.Fault.faultstring + (body.Fault.detail ? ': ' + body.Fault.detail : ''), body.Fault.faultcode);
    error.root = root;
    throw error;
  }
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, first, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  ...
  var isSchemaNamespaceIgnored = this.shouldIgnoreScheamNamespace(schema);

  var xmlnsAttrib = '';
  if (xmlns && first) {

    if (prefixNamespace && (!isSchemaNamespaceIgnored || this.options.ignoredNamespaces.indexOf(namespace) === -1)) {
  ...
  var ns = '';
  if (prefixNamespace && ((qualified || first) || soapHeader) && (!isSchemaNamespaceIgnored || this.options.ignoredNamespaces.indexOf(namespace) === -1)) {
    // prefix element
